    ...
    return (
        <div className='bg-blue-200'>
            <div className="p-8 max-w-6xl mx-auto min-h-screen">
                <h1 className="text-3xl font-bold text-blue-900 text-center mb-8">Our Beautiful Birds</h1>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {birds.map((bird) => (
                        <div key={bird.id} className="border p-4 rounded-xl bg-blue-900 shadow-sm">
                            <img
                                src={`/img/${bird.image_url}`}
                                alt={bird.name}
                                className="w-full h-48 object-cover rounded-lg"
                            />
                            <h2 className="text-xl font-bold text-yellow-400 mt-2">{bird.name}</h2>
                            <p className="text-sm text-white font-semibold">{bird.breed} • {bird.age}</p>
                            <p className="text-blue-200 mt-2 text-sm">{bird.description}</p>
